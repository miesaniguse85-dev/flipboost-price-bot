...
async def handle(client, msg):
    ...
    if text in ("/start", "/help"):
        await send(client, chat_id,
            ...
            "Commands:\n"
            "/setprice 250g 600     — አንድ ዋጋ ቀይር\n"
            "/setall 525 1000 1900  — ሦስቱን አንድ ጊዜ ቀይር\n"
            "/prices                — አሁን ያሉ ዋጋዎች\n\n"
            "setall ቅደም ተከተሉ: 250g · 500g · 1kg")
        ...
        await send(client, chat_id,
            ...
            f"  500g --> ETB {p['500g']:,}\n"
            f"  1kg  --> ETB {p['1kg']:,}\n\n"
            "Site ቀጣይ ጊዜ ሲጫን ይዘምናል")

    elif text.startswith("/setall"):
        parts = text.split()
        if len(parts) != 4:
            await send(client, chat_id,
                "Usage: /setall 525 1000 1900\n"
                "(ቅደም ተከተሉ: 250g · 500g · 1kg)")
            return
        sizes = ["250g", "500g", "1kg"]
        results = []
        ...
        for i, size in enumerate(sizes):
            try:
                ...
                results.append(f"❌ {parts[i+1]} — ቁጥር መሆን አለበት።")
                continue
            ok, reply = await set_price(client, size, price, admin_name)
            results.append(reply)
        await send(client, chat_id, "\n\n".join(results))

    elif "/setprice" in text:
        # Support multiple /setprice commands in one message
        lines = [l.strip() for l in text.splitlines() if l.strip().startswith("/setprice")]
        if not lines:
            lines = [text]
        results = []
        for line in lines:
            parts = line.split()
            if len(parts) != 3:
                results.append("❌ Usage: /setprice 250g 600")
                continue
            size = parts[1].lower().strip()
            if size not in SIZE_KEY:
                results.append(f"❌ {size} — 250g, 500g ወይም 1kg መሆን አለበት።")
                continue
            try:
                price = int(parts[2].replace(",", ""))
            except ValueError:
                results.append(f"❌ {parts[2]} — ቁጥር መሆን አለበት።")
                continue
            if not (1 <= price <= 100000):
                results.append(f"❌ {price} — ከ 1 እስከ 100,000 ETB መሆን አለበት።")
                continue
            admin_name = msg["from"].get("first_name", "admin")
            ok, reply = await set_price(client, size, price, admin_name)
            results.append(reply)
